...
# contact
################################################################################
struct PolyHalfSpace181{T,D,NP} <: Node{T}
    name::Symbol
    parent_name::Symbol
...
function PolyHalfSpace181(parent_body::Body181{T}, Ac::AbstractMatrix, bc::AbstractVector;
    ...
    return PolyHalfSpace181(parent_name, friction_coefficient, Ap, bp, Ac, bc;
        name=name)
end

function PolyHalfSpace181(
        parent_name::Symbol,
    ...
    np = size(Ap, 1)
    index = NodeIndices181()
    return PolyHalfSpace181{T,d,np}(
        name,
        parent_name,
        index,
        [friction_coefficient],
...
primal_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = D + 1 # x, ϕ
cone_dimension(contact::PolyHalfSpace181{T,D,NP}) where {T,D,NP} = 1 + 1 + 2 + NP + 1 # γ ψ β λp, λc
variable_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = primal_dimension(contact) + 2 * cone_dimension(contact)
equality_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = primal_dimension(contact) + cone_dimension(contact)
optimality_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = primal_dimension(contact)
slackness_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = cone_dimension(contact)
equality_dimension(contact::PolyHalfSpace181{T,D}) where {T,D} = optimality_dimension(contact) + slackness_dimension(contact)


function parameter_dimension(contact::PolyHalfSpace181{T,D}) where {T,D}
...
function unpack_variables(x::Vector{T}, contact::PolyHalfSpace181{T,D,NP}) where {T,D,NP}
...
function get_parameters(contact::PolyHalfSpace181{T,D}) where {T,D}
    θ = [
...
function set_parameters!(contact::PolyHalfSpace181{T,D,NP}, θ) where {T,D,NP}
...
function unpack_parameters(θ::Vector, contact::PolyHalfSpace181{T,D,NP}) where {T,D,NP}
    @assert D == 2
    NC = 1
    off = 0
...
function residual!(e, x, θ, contact::PolyHalfSpace181{T,D,NP},
        pbody::Body181) where {T,D,NP}
    NC = 1
    # unpack parameters
