...
function Halfspace177(parent_body::Body177{T}, Ac::AbstractMatrix, bc::AbstractVector;
    ...
    return Halfspace177(parent_name, friction_coefficient, Ap, bp, Ac, bc;
        name=name)
end

function Halfspace177(
        parent_name::Symbol,
...
primal_dimension(contact::Halfspace177{T,D}) where {T,D} = D + 1 # x, ϕ
cone_dimension(contact::Halfspace177{T,D,NP}) where {T,D,NP} = 1 + 1 + 2 + NP + 1 # γ ψ β λp, λc
variable_dimension(contact::Halfspace177{T,D}) where {T,D} = primal_dimension(contact) + 2 * cone_dimension(contact)
equality_dimension(contact::Halfspace177{T,D}) where {T,D} = primal_dimension(contact) + cone_dimension(contact)
optimality_dimension(contact::Halfspace177{T,D}) where {T,D} = primal_dimension(contact)
slackness_dimension(contact::Halfspace177{T,D}) where {T,D} = cone_dimension(contact)
equality_dimension(contact::Halfspace177{T,D}) where {T,D} = optimality_dimension(contact) + slackness_dimension(contact)


function parameter_dimension(contact::Halfspace177{T,D}) where {T,D}
...
function unpack_variables(x::Vector{T}, contact::Halfspace177{T,D,NP}) where {T,D,NP}
...
function get_parameters(contact::Halfspace177{T,D}) where {T,D}
    θ = [
...
function set_parameters!(contact::Halfspace177{T,D,NP}, θ) where {T,D,NP}
...
function unpack_parameters(θ::Vector, contact::Halfspace177{T,D,NP}) where {T,D,NP}
    @assert D == 2
    NC = 1
    off = 0
...
function contact_residual!(e, x, θ, contact::Halfspace177{T,D,NP},
