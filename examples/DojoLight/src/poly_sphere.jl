...
# contact
################################################################################
struct PolySphere1160{T,D,NP} <: Node{T}
    name::Symbol
    parent_name::Symbol
    child_name::Symbol
...
function PolySphere1160(parent_body::Body1160{T}, child_body::Body1160{T};
    ...
    return PolySphere1160(parent_name, child_name, friction_coefficient, Ap, bp, radc, offc;
        name=name)
end

function PolySphere1160(
        parent_name::Symbol,
        child_name::Symbol,
...
function unpack_variables(x::Vector, contact::PolySphere1160{T,D,NP}) where {T,D,NP}
...
function get_parameters(contact::PolySphere1160{T,D}) where {T,D}
    θ = [
...
function set_parameters!(contact::PolySphere1160{T,D,NP}, θ) where {T,D,NP}
...
function unpack_parameters(θ::Vector, contact::PolySphere1160{T,D,NP}) where {T,D,NP}
    @assert D == 2
    off = 0
...
function residual!(e, x, θ, contact::PolySphere1160{T,D,NP},
        pbody::Body1160, cbody::Body1160) where {T,D,NP}
