...
function Body183(timestep, mass, inertia::Matrix,
        A_colliders::Vector{Matrix{T}},
        b_colliders::Vector{Vector{T}};
        gravity=-9.81,
        name::Symbol=:body,
        index::NodeIndices183=NodeIndices183()) where T
...
primal_dimension(body::Body183{T,D}) where {T,D} = 3
cone_dimension(body::Body183{T,D}) where {T,D} = 0
variable_dimension(body::Body183{T,D}) where {T,D} = primal_dimension(body) + 2 * cone_dimension(body)
optimality_dimension(body::Body183{T,D}) where {T,D} = primal_dimension(body)
slackness_dimension(body::Body183{T,D}) where {T,D} = cone_dimension(body)
equality_dimension(body::Body183{T,D}) where {T,D} = optimality_dimension(body) + slackness_dimension(body)

function parameter_dimension(body::Body183{T,D}) where {T,D}
    @assert D == 2
    nq = 3 # configuration
    nv = 3 # velocity
    nu = 3 # input
...
function unpack_variables(x::Vector, body::Body183{T}) where T
    return x
end

function get_parameters(body::Body183{T,D}) where {T,D}
    @assert D == 2
    pose = body.pose
...
function set_parameters!(body::Body183{T,D}, θ) where {T,D}
...
function unpack_parameters(θ::Vector, body::Body183{T,D}) where {T,D}
    @assert D == 2
    off = 0
...
function find_body(bodies::AbstractVector{<:Body183}, name::Symbol)
    idx = findfirst(x -> x == name, getfield.(bodies, :name))
    return bodies[idx]
end

function residual!(e, x, θ, body::Body183)
    index = body.index
