...
# contact
################################################################################
struct SphereSphere1160{T,D} <: Node{T}
    name::Symbol
    parent_name::Symbol
    child_name::Symbol
...
function SphereSphere1160(parent_body::Body{T}, child_body::Body{T};
    ...
    return SphereSphere1160(parent_name, child_name, friction_coefficient,
        radp, offp, radc, offc;
        name=name)
end

function SphereSphere1160(
        parent_name::Symbol,
        child_name::Symbol,
...
function unpack_variables(x::Vector, contact::SphereSphere1160{T,D}) where {T,D}
...
function get_parameters(contact::SphereSphere1160{T,D}) where {T,D}
    θ = [
...
function set_parameters!(contact::SphereSphere1160{T,D}, θ) where {T,D}
...
function unpack_parameters(θ::Vector, contact::SphereSphere1160{T,D}) where {T,D}
    @assert D == 2
    off = 0
