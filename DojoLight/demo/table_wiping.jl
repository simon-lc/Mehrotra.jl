...
function dynamics(solver, z, u; timestep=0.10, mass=1.0, gravity=1.0*-9.81, friction_coefficient=1.1, side=0.5)
    ...
    v25dz = solver.data.solution_sensitivity[solver.indices.primals, 1:6]
    p3dz = [I(3) zeros(3,3)] + timestep * v25dz
    z1dz = [p3dz; v25dz]
...
function cost(z0, z, u, λ, zref, g, H;
        Qz=Diagonal([1e0,1e0,1e1,1e-1,1e-1,1e-1]),
        Qu=Diagonal(1e-4ones(3)),
        gradient::Bool=true,
        hessian::Bool=true,
    ...
    for i = 1:N
        ...
        if hessian
            ...
            H[indλ, indz] += -I
            H[indz, indλ] += -I
            (i > 1) && (H[indλ, indz .- 15] += dzdyn)
            (i > 1) && (H[indz .- 15, indλ] += dzdyn')
        end
    end
...
function newton_method(z0, z, u, λ, zc, uc, λc, zref, g, H; iterations=20)
    for i = 1:iterations
        c, g, H, violation = cost(z0, z, u, λ, zref, g, H)
        Δ = - H \ g
        ...
        for j = 1:3
            ...
            c_candidate, _, _, violation_candidate = cost(z0, zc, uc, λc, zref, g, H, gradient=false, hessian=false)
...
zc = [zeros(6) for i = 1:N]
uc = [zeros(3) for i = 1:N]
λc = [zeros(6) for i = 1:N]
...
@elapsed z, u, λ = newton_method(z0, z, u, λ, zc, uc, λc, zref, g, H)
0.05/20
...
for i = 1:length(ztraj)
    MeshCat.atframe(anim, i) do
        settransform!(vis[:particle], MeshCat.Translation(SVector{3}(ztraj[i][1:3])))
    end
end
MeshCat.setanimation!(vis, anim)
