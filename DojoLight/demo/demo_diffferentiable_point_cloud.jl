...
lookat = [0,0,0.0]
up = [0,1,0.0]
E = [4.0*[0, cos(α), sin(α)] for α in range(0.2π, 0.8π, 5)]
z_nominal = zeros(6)
...
################################################################################
# recover polytope bundle from point cloud
################################################################################
A0 = [
    +1.0 -0.2;
    +0.0 +1.0;
...
θtruth, bundle_dimensions_truth = pack_halfspaces([A0, A1, A2], [b0, b1, b2], [zeros(2), zeros(2), zeros(2)])
point_cloud_loss(glvis, p1, p2, resolution, eyeposition, lookat, up, WC, E, θtruth, bundle_dimensions_truth)
...
# build_2d_convex_bundle!(glvis, :root, θinit, bundle_dimensions)
...
# loss
local_loss(θ) = point_cloud_loss(glvis, p1, p2, resolution, eyeposition, lookat, up, WC, E, θ, bundle_dimensions)
local_initial_invH(θ) = zeros(np * (2n+2), np * (2n+2)) + 1e-3*I

# initialization
n = 5
bundle_dimensions = [n,n,n]
...
point_cloud_loss(glvis, p1, p2, resolution, eyeposition, lookat, up, WC, E, θinit, bundle_dimensions)
